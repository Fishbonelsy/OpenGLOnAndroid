package com.live.longsiyang.openglonandroid.picture.glrender.stlmodule;
...
public class STLGLRender implements GLSurfaceView.Renderer {
    ...
    public STLGLRender(Context context) {
        try {
            model = new STLModel();
            InputStream is = context.getAssets().open("test.stl");
            model.parserBinStl(is);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
